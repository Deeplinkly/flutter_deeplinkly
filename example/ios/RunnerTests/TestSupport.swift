...
enum DeeplinklyTestSupport {
    ...
    ///
    /// `LinkDomains` reads `Bundle.main`, which in a host-app unit test bundle
    /// is the Runner app — so this is a fixture, not a constant, and the tests
...
/// A `FlutterBinaryMessenger` that records what was sent instead of crossing
/// the engine boundary.
///
/// Now only needed for the plugin's own bridge
/// (`MethodChannelDeepLinkListener`), which is the one piece that still speaks
/// to a channel. `FlutterMethodChannel` is concrete and cannot be usefully
/// subclassed, but it accepts any messenger — so the fake goes one level down.
final class FakeBinaryMessenger: NSObject, FlutterBinaryMessenger {
    ...
    /// Decoded method calls, in the order they were sent.
