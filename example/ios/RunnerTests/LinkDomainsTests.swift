...
///
/// `LinkDomains` reads `Bundle.main`, which in a host-app test bundle is the
/// Runner app — so these run against `example/ios/Runner/Info.plist`, which
/// configures `DeeplinklyLinkDomains = [example.deeplinkly.com]`. That makes
/// the *configured* branch of both functions testable and the *unconfigured*
/// branch unreachable; see `SEAM_TESTS.md`.
final class LinkDomainsTests: XCTestCase {

    private let domain = DeeplinklyTestSupport.configuredDomain

    /// Guards the fixture the rest of the suite assumes. If the example app's
    /// Info.plist changes, this fails first and explains why.
