...
/// The plugin's end of the funnel: the one piece that still speaks to a
/// `FlutterMethodChannel`, and the only thing that stays behind when the rest
/// moves into a native SDK.
final class MethodChannelDeepLinkListenerTests: XCTestCase {
    ...
    /// Wiring it to the funnel is what the plugin actually does.
    func testDeliversThroughTheFunnel() {
        DeeplinklyTestSupport.reset()
        defer { DeeplinklyTestSupport.reset() }
